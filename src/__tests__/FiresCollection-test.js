...
import { set as setSettings, reset as resetSettings } from "../settings";
...
describe("FiresCollection", () => {
  ...
  describe("firesCollection.createWithId(doc)", () => {
    ...
    it("should throw if id already exists", async () => {
      ...
      await expect(
        collection.createWithId("the id", { a: 3 })
      ).rejects.toMatchInlineSnapshot(`[Error: col.the id already exists]`);
    });
  });
  ...
  describe("firesCollection.subscribe", () => {
    ...
    describe("firesCollection._subscribe", () => {
      it("should call onNext", async done => {
        $mockCollection.onSnapshot.mockImplementation(
          (options, onNext, onError) => {
    ...
    describe("firesCollection.subscribeIncludingMetadata(queryObj, onNext, onError)", () => {
      it("should call _subscribe with correct options", async () => {
        ...
        await collection.subscribeIncludingMetadata(
          { country: [">", "us"] },
          onNext,
          onError
        );
